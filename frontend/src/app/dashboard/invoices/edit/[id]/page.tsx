...
export default function EditInvoicePage() {
  ...
  const fetchInvoice = async () => {
    try {
      ...
      setWorkOrder(invoice.workOrder || "");
      setBillingPeriodFrom(invoice.billingPeriod?.from ? new Date(invoice.billingPeriod.from).toISOString().split("T")[0] : "");
      setBillingPeriodTo(invoice.billingPeriod?.to ? new Date(invoice.billingPeriod.to).toISOString().split("T")[0] : "");
      setStatus(invoice.status || "draft");
      setNotes(invoice.notes || "");
      setItems(invoice.items || []);
      setMaterialCharges(invoice.materialCharges || 0);
      setManagementChargesPercentage(invoice.managementCharges?.percentage || 10);
      setCgstPercentage(invoice.cgst?.percentage || 9);
      setSgstPercentage(invoice.sgst?.percentage || 9);
